...
router.put('/5', tokenCheck, async (req: ReqUser, res) => {
    ...
    try {
        ...
        for await (const userdef of definitions) {
            ...
            const userMetricResults = await userMetricsDB.update({ userid, valueid: userdef.valueid, personal_definition: userdef.personal_definition }, userid, userdef.valueid);
    ...
    try {
        ...
        for await (const userRating of userRatings) {
            ...
            const userMetricResults = await userMetricsDB.update({
                userid, valueid: userRating.valueid,
                congruence_rating: userRating.rating
            }, userid, userRating.valueid);
...
router.put('/8', tokenCheck, async (req: ReqUser, res) => {
    ...
    try {
        ...
        if (!userPriorities.length) { // ? removed length check !== 10
            return res.status(400).json({ message: "Fill out everything!" })
        }
        ...
        for await (const userPriority of userPriorities) {
            ...
            const userMetricResults = await userMetricsDB.update({ priority: userPriority.score }, userid, userPriority.valueid);
